...
static void msm_flush_console(void)
{
	...
	if (console_trylock())
		pr_emerg("restart: Console was locked! Busting\n");
	else
		pr_emerg("restart: Console was locked!\n");
	console_unlock();
...
static void set_modem_efs_sync(void)
{
	...
	pr_debug("%s: wait for modem efs_sync\n", __func__);
}
...
static void check_modem_efs_sync_timeout(unsigned timeout)
{
	...
	if (timeout <= 0)
		...
		pr_debug("%s: modem efs_sync done.\n", __func__);
}
...
static int set_reset_reason(unsigned int reason)
{
	...
	if (pnum < 0) {
		pr_debug("Unknonwn partition number for misc partition\n");
		return -1;
	}
	...
	if (IS_ERR(filp) || (!filp)) {
		pr_err("Unable to open misc partition file: %s\n", filename);
		return PTR_ERR(filp);
	}
